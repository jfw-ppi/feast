...
def get_app(
    ...
    if fs_cfg is not None:
        batching_cfg = getattr(fs_cfg, "offline_write_batching", None)
    ...
    async def push(request: PushFeaturesRequest) -> None:
        ...
        async def _push_with_to(push_to: PushMode) -> None:
            ...
            if push_to in (PushMode.ONLINE, PushMode.ONLINE_AND_OFFLINE) and (
                ...
                await run_in_threadpool(lambda: store.push(**push_params))
            ...
            try:
                with self._lock:
                    for key, dfs in list(self._buffers.items()):
                        if not dfs:
                            continue
                        last = self._last_flush[key]  # this will also init the default timestamp
                        age = now - last
